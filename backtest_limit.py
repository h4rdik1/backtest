...
def simulate_trades(df_ltf: pd.DataFrame, df_bias: pd.DataFrame, df_htf: pd.DataFrame = None) -> Dict:
    """
    Core simulation with Phase 2 filters and precise 50% FVG Limit Entries.
    """
    ...
    for i in range(25, len(df_ltf)):
        ...
        # State tracking for active limit orders
        # If we have an active limit order and it gets hit this candle
        if not in_trade and 'active_limit' in locals() and active_limit is not None:
            # Check if limit expires (e.g., 10 bars passed)
            if i - active_limit['index'] > BIAS_EXPIRY_BARS:
                ...
                if active_limit['direction'] == 1 and row['low'] <= active_limit['entry']:
                    in_trade = True
                    current_trade = active_limit['trade_template']
                    current_trade['Entry Time'] = timestamp.strftime("%Y-%m-%d %H:%M")
                    # We might get better fill, but let's assume exact limit fill
                    active_limit = None
        ...
        if in_trade:
            # Check for exit (Phase 3: Break Even Logic)
        ...
        if is_bull_setup:
            ...
            if pd.isna(fvg_top) or pd.isna(fvg_btm): continue
            entry = fvg_top - ((fvg_top - fvg_btm) / 2)
            
            # Phase 3: True Structural Stop
            stop = df_ltf['high'].iloc[max(0, i-10):i].max() + (avg_range_ltf.iloc[i] * 0.2)
            ...
            active_limit = {
                ...
                'trade_template': {
                    ...
                    "Stop $": round(stop, 4), "Target $": round(target, 4), 
                    "be_trigger": round(be_trigger, 4), "is_be": False,
                    "Entry Type": "Phase 3 Bear Limit (50%)", "account": account
                }
            }
            funnel["CISD Triggered"] += 1 # Repurposed as Valid Limit Set
...
def print_stats(trades: List[Dict], funnel: Dict = None, label: str = "BACKTEST"):
    ...
    # Calculate win rate effectively ignoring BE trades from the base, or counting them uniquely
    total_effective = len(wins) + len(losses)
    wr = (len(wins) / total_effective * 100) if total_effective > 0 else 0
    gross_wins = wins["pnl"].sum()
    gross_losses = abs(losses["pnl"].sum())
    ...
    for r in results:
        if r == "WIN":
            ...
            curr_win = 0
            max_loss_streak = max(max_loss_streak, curr_loss)
        # BE breaks streaks or acts neutral. Let's say it acts neutral (keeps streaks alive but doesn't increment)
