...
def classify_regime(df_daily: pd.DataFrame, entry_time) -> str:
    ...
    entry_ts = (
        pd.Timestamp(entry_time, tz="UTC")
...
def monte_carlo(
    trades: List[Dict],
    n_simulations: int = MONTE_CARLO_SIMS,
    ...
    if not trades:
        return {
            "profitable_pct": 0,
            "median_equity": account_size,
            "p5_equity": account_size,
            "p95_equity": account_size,
            "median_max_dd": 0,
            "verdict": "NO TRADES",
        }
...
def walk_forward_split(
    ...
    verdict = (
        "PASS"
        if (wr_dev <= WALK_FORWARD_TOLERANCE and pf_dev <= WALK_FORWARD_TOLERANCE)
        else "FAIL"
    )
...
def filter_contribution_analysis(
    df_ltf: pd.DataFrame,
    df_bias: pd.DataFrame,
    df_htf: pd.DataFrame,
    simulate_fn: Callable,
    ...
    for stage in FILTER_STAGES:
        ...
        flag = (
            ...
            and pnl < prev_pnl
            and pf < (results[-1]["pf"] if results else 999)
            else ""
        )
...
def print_enhanced_stats(
    ...
    sample_verdict = (
        "✅ VALID"
        if len(df_t) >= MIN_TRADES_FOR_VALIDITY
        else f"⚠️  LOW (need {MIN_TRADES_FOR_VALIDITY})"
    )
    ...
    print("\n  MONTHLY BREAKDOWN:")
    print(
        f"    {'Month':<10} {'Trades':>6} {'Wins':>5} {'Loss':>5} {'PnL':>10} {'WR%':>6}"
    )
    print("    " + "-" * 46)
    for m in mb["monthly_data"]:
        print(
            f"    {m['month']:<10} {m['trades']:>6} {m['wins']:>5} {m['losses']:>5} ${m['pnl']:>8.2f} {m['wr']:>5.1f}%"
        )
    ...
    if run_mc and len(trades) >= 5:
        ...
        print(
            f"    Profitable sims   : {mc['profitable_pct']:.1f}%   [{'PASS' if mc['profitable_pct'] >= EDGE_REAL_THRESHOLD * 100 else 'FAIL'} — need ≥{EDGE_REAL_THRESHOLD * 100:.0f}%]"
        )
...
def print_walk_forward(wf_result: Dict, label: str = ""):
    ...
    print(
        f"    PF deviation  : {wf_result['pf_deviation']:.1f}%  (tolerance: {WALK_FORWARD_TOLERANCE * 100:.0f}%)"
    )
...
def print_filter_contribution(results: List[Dict]):
    ...
    print(
        f"    {'Stage':<25} {'Trades':>7} {'WR%':>6} {'PF':>6} {'PnL':>10} {'Change':>8} {'':>10}"
    )
    print("    " + "-" * 75)
    for r in results:
        change_str = f"${r['change']:>+7.2f}" if r["change"] != 0 else "    —"
        print(
            f"    {r['name']:<25} {r['trades']:>7} {r['wr']:>5.1f}% {r['pf']:>6.2f} ${r['pnl']:>9.2f} {change_str} {r['flag']}"
        )
...
def compare_live_vs_backtest(backtest_wr: float) -> Dict:
    ...
    deviation = (
        abs(live_wr - backtest_wr) / backtest_wr * 100 if backtest_wr > 0 else 100
    )
